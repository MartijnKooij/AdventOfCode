    ...
    public static class Program
    {
        public static void Main()
        {
            ...
            while (true)
            {
                if (RunFight(boost))
                {
                    Console.WriteLine($"Reindeer saved with {boost} boost");
                    break;
                }
                boost += 1;
            }
        ...
        private static bool RunFight(int boost)
        {
            ...
            while (units.Where(u => u.Count > 0).Select(u => u.UnitType).Distinct().Count() > 1)
            {
                ...
                if (AttackTargets(units)) continue;
                Console.WriteLine("Mexican standoff detected");
                return false;
            }

            var winningUnit = units.First(u => u.Count > 0);
            var winningArmySize = units.Sum(u => u.Count);
            Console.WriteLine($"The winning army {winningUnit.UnitType} has {winningArmySize} units left using {boost} boost");

            return winningUnit.UnitType == UnitType.ImmuneSystem;
        }
        ...
        private static bool AttackTargets(IReadOnlyCollection<Unit> units)
        {
            var didAnyGroupDealDamage = false;
                    ...
                    )
            {
                ...
                var enemyCountBeforeHit = enemy.Count;
                enemy.Hit(unit.AttackType, unit.EffectivePower);

                if (enemy.Count != enemyCountBeforeHit)
                {
                    //Reset the fight to prevent infinite loops
                    didAnyGroupDealDamage = true;
                }
        ...
        private static List<Unit> PopulateUnits(int boost)
        {
            return new List<Unit>{
                new Unit(UnitType.ImmuneSystem, 8808, 5616, new[] { AttackType.Ice }, new[] { AttackType.Radiation }, 5 + boost, AttackType.Bludgeoning, 10),
                new Unit(UnitType.ImmuneSystem, 900, 13511, new AttackType[] { }, new[] { AttackType.Radiation }, 107 + boost, AttackType.Radiation, 20),
                new Unit(UnitType.ImmuneSystem, 581, 10346, new[] { AttackType.Slashing }, new[] { AttackType.Radiation }, 140 + boost, AttackType.Fire, 14),
                new Unit(UnitType.ImmuneSystem, 57, 9991, new[] { AttackType.Slashing, AttackType.Radiation, AttackType.Fire }, new[] { AttackType.Bludgeoning }, 1690 + boost, AttackType.Fire, 4),
                new Unit(UnitType.ImmuneSystem, 4074, 6549, new AttackType[] { }, new[] { AttackType.Fire }, 15 + boost, AttackType.Radiation, 2),
                new Unit(UnitType.ImmuneSystem, 929, 5404, new[] { AttackType.Bludgeoning, AttackType.Radiation }, new AttackType[] {  }, 45 + boost, AttackType.Fire, 16),
                new Unit(UnitType.ImmuneSystem, 2196, 3186, new[] { AttackType.Radiation }, new[] { AttackType.Fire }, 10 + boost, AttackType.Fire, 11),
                new Unit(UnitType.ImmuneSystem, 4420, 9691, new[] { AttackType.Fire }, new[] { AttackType.Radiation }, 21 + boost, AttackType.Fire, 7),
                new Unit(UnitType.ImmuneSystem, 3978, 2306, new AttackType[] {  }, new[] { AttackType.Ice, AttackType.Radiation }, 4 + boost, AttackType.Fire, 12),
                new Unit(UnitType.ImmuneSystem, 1284, 4487, new AttackType[] {  }, new[] { AttackType.Radiation, AttackType.Bludgeoning }, 32 + boost, AttackType.Slashing, 19),
        ...
        private static List<Unit> PopulateTestUnits(int boost)
        {
            return new List<Unit>{
                new Unit(UnitType.ImmuneSystem, 17, 5390, new AttackType[] {  }, new[] { AttackType.Radiation, AttackType.Bludgeoning }, 4507 + boost, AttackType.Fire, 2),
                new Unit(UnitType.ImmuneSystem, 989, 1274, new[] { AttackType.Fire }, new[] { AttackType.Bludgeoning, AttackType.Slashing }, 25 + boost, AttackType.Slashing, 3),
